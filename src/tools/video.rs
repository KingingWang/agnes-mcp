...
#[macros::mcp_tool(
    ...
    description = "Generate video with the Agnes-Video-V2.0 model. Supports text-to-video, image-to-video (single image), multi-image, and keyframe animation. Generation is asynchronous: by default it returns a task id immediately; set wait=true to poll until completion and return the video URL.",
...
impl GenerateVideoTool {
    ...
    fn build_body(params: &GenerateVideoToolParams) -> serde_json::Value {
        validate_num_frames(params.num_frames).ok();

        let mut body = serde_json::json!({
            "model": AgnesClient::video_model(),
...
impl Tool for GenerateVideoTool {
    ...
    async fn execute(
        ...
        let content = render_task_result(&final_response, &task_id, status, video_url.as_deref());
...
#[macros::mcp_tool(
    ...
    description = "Retrieve the status (and result, when complete) of an Agnes video generation task by its task id. Set wait=true to poll until the task completes, fails, or times out.",
    destructive_hint = false,
...
impl Tool for VideoStatusTool {
    ...
    async fn execute(
        ...
        let content = render_task_result(&response, &params.task_id, status, video_url.as_deref());
...
mod tests {
    ...
    fn build_body_text_to_video() {
        ...
        let body = GenerateVideoTool::build_body(&params);
        assert_eq!(body["model"], AgnesClient::video_model());
